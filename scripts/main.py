...
# 	distances_to_sample_window_clustering = map(lambda curr_target_window_clustering: norm(curr_target_window_clustering - sample_window_clustering), target_clusterings)
# 	print("Done in %0.3fs." % (time() - t0))

# 	# indexed_distances = np.stack((distances_to_target_clustering,np.arange(len(distances_to_target_clustering))),1)
# 	figure = plt.figure()
# 	plt.plot(distances_to_sample_window_clustering)
