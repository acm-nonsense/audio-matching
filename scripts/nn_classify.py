...
def process_files(output_root):
    files_raw = os.listdir(os.path.join(output_root,'npz'))
    files = []
    for i in range(len(files_raw)):
            npz_type = files_raw[i][-8:-4]
            if npz_type == 'assm':
                files.append(files_raw[i])
        ...
        for i in range(len(files)):
            ...
            if npz_type == 'assm':
                with np.load(os.path.join(output_root,'npz',files[i])) as ssm_file:
                    ...
                    if (ssm.shape[0] < first_ssm.shape[0]):
                        values[i] = np.pad(ssm,((0,1),(0,1)),mode='constant')
                    else:
                        values[i] = ssm[:first_ssm.shape[0],:first_ssm.shape[0]]
                    labels[i] = files[i][:-15]
